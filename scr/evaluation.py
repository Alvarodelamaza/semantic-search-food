...
# Example usage with integer product IDs:
def run_evaluation_example():
    # Sample data - 3 queries
    queries = ["query for product 0", "query for product 1", "query for product 2"]
    
    # Example search results (ranked lists of product IDs)
    # Note: True product IDs are 0, 1, and 2 respectively
    search_results = [
        [0, 4, 7],    # perfect match at rank 1 for query 0
        [5, 1, 8],    # correct product at rank 2 for query 1
        [7, 9, 10]    # correct product not in top results for query 2
    ]
    
    # Evaluate
    metrics = evaluate_search_system(queries, search_results)
    
    # Print results
    for metric, value in metrics.items():
        print(f"{metric}: {value:.4f}")

# Full evaluation with your 100 queries and products
